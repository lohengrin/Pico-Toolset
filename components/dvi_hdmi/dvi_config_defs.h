...
// PATCH (hdmi-audio): compiles in HDMI data-island audio support (CEA-861
// InfoFrames/ACR/audio-sample packets riding the horizontal blanking
// interval -- see dvi_init()/dvi_set_audio_freq() in dvi.h) when set to 1.
// Default 0 keeps every file byte-for-byte the original vendored (pure-DVI,
// video-only) behavior -- this is the regression firewall behind this
// component's own PICO_TOOLSET_DVI_HDMI_AUDIO CMake option (see
// CMakeLists.txt in this directory).
#ifndef DVI_ENABLE_AUDIO
#define DVI_ENABLE_AUDIO 0
#endif

// PATCH (irq-stats): compiles in a cumulative-microseconds counter around
// dvi_dma_irq_handler()'s body (see dvi.c/dvi.h) so a caller on the other
// core can read how much of core1's per-scanline deadline that handler
// actually uses -- the exact headroom question DVI_ENABLE_AUDIO's added
// data-island work needs answered on real hardware, but independent of it:
// build with audio off to get a baseline, on to see the delta, both under
// this same flag. Default 0 keeps dvi.c/dvi.h byte-for-byte the original
// vendored behavior -- this is the regression firewall behind this
// component's own PICO_TOOLSET_DVI_HDMI_IRQ_STATS CMake option (see
// CMakeLists.txt in this directory).
#ifndef DVI_ENABLE_IRQ_STATS
#define DVI_ENABLE_IRQ_STATS 0
#endif
